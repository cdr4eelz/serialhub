...
void handleCmd() {
  ...
  if (handshake == '9') {  // Ascii code 57 for numerical 9
    Serial.flush();
    tock = 0;  // Cancel timer if running
  } else if (handshake == '8') {  // Ascii code 56 for numerical 8
    // Serial.println(F("Start"));
    needRep = numRep;  // Reset repeat counter to beginning
    tock = millis(); // Set timer "tock" to fire very soon
  } else if (handshake == '7') {  // Ascii code 55 for numerical 7
    needRep = 1000;  // Run for long time
    tock = millis(); // Set timer "tock" to fire right away
  } else if (handshake == '6') { // Ascii code 54 for 6
...
void handleTock() {
  ...
  if (tick > tock) { // Has timer "tock" expired?
    if (needRep) {
      needRep--;
      rep_cb();
    }
    if (needRep) { // Perform callback & check if more
...
void loop() {
  ...
  if (tock) { // Is a timer event pending?
    handleTock();
  }
}
